import {AuthKitStateInterface, AuthStateUserObject} from './authTypes';
...
class TokenObject {
  ...
  initialCookieToken_(): AuthKitStateInterface {
    const authToken = getCookie(this.authStorageName);
    const authTokenType = getCookie(this.authStorageTypeName);
    const authTokenTime = getCookie(this.authTimeStorageName);
    const stateCookie = getCookie(this.stateStorageName);

    const refreshToken = this.isUsingRefreshToken &&
    this.refreshTokenName != null ? getCookie(this.refreshTokenName) : null;

    const refreshTokenTime = this.isUsingRefreshToken &&
    this.refreshTokenTimeName != null ?
      getCookie(this.refreshTokenTimeName) : null;
  ...
  setCookieToken_(
      ...
      refreshTokenExpiresAt: Date | null,
      authState: AuthStateUserObject|null): void {
    setCookie(this.authStorageName, authToken, {
      expires: expiresAt,
      domain: this.cookieDomain,
      secure: this.cookieSecure,
    });
    setCookie(this.authStorageTypeName, authTokenType, {
      expires: expiresAt,
      domain: this.cookieDomain,
      secure: this.cookieSecure,
    });
    setCookie(this.authTimeStorageName, expiresAt.toISOString(), {
      expires: expiresAt,
      domain: this.cookieDomain,
      secure: this.cookieSecure,
    });
    if (authState) {
      setCookie(this.stateStorageName, JSON.stringify(authState), {
        expires: expiresAt,
        domain: this.cookieDomain,
        secure: this.cookieSecure,
      });
    ...
    if (this.isUsingRefreshToken && !!this.refreshTokenTimeName &&
      !!refreshTokenExpiresAt) {
      setCookie(this.refreshTokenTimeName,
  ...
  removeCookieToken_(): void {
    removeCookie(this.authStorageName, {
      domain: this.cookieDomain,
      secure: this.cookieSecure,
    });
    removeCookie(this.authTimeStorageName, {
      domain: this.cookieDomain,
      secure: this.cookieSecure,
    });
    removeCookie(this.authStorageTypeName, {
      domain: this.cookieDomain,
      secure: this.cookieSecure,
    });
    removeCookie(this.stateStorageName, {
      domain: this.cookieDomain,
      secure: this.cookieSecure,
    });
    if (this.isUsingRefreshToken && !!this.refreshTokenName) {
    ...
    if (this.isUsingRefreshToken && !!this.refreshTokenTimeName) {
      removeCookie(this.refreshTokenTimeName, {
        domain: this.cookieDomain,
        secure: this.cookieSecure,
      });
    }
